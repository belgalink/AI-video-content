...
V12="/tmp/newscenes_v12"
V11="/tmp/newscenes_v11"
TMP="/tmp/asm_v15"; os.makedirs(TMP,exist_ok=True)
...
# ============ VIDEO ============
# Scene 1: regenerated without video_references (start_image + audio_references only),
# so the plate comes out correct from generation -- no overlay patch needed anymore.
# Scene 3: regenerated with a corrected prompt -- suitcase now goes from the ground
# INTO the trunk in one forward motion (previous version showed it coming out of the
# trunk first, then going back in, which read as backwards).
# Scene 2 (v13's no-handshake version), scenes 4/6 (v11 cinematic), scene 5 (v12,
# still needs its plate patch) are unchanged.
SCENES=[
    ("/tmp/scene1_fixed.mp4", 2.1667, None, None),
    ("/tmp/scene2_nohandshake.mp4", 1.9333, None, None),
    ("/tmp/scene3_fixed.mp4", 1.7333, None, None),
...
bed1=f"{TMP}/bed1.wav"; synth_bed("brown", clip1_len, 250, 100, 0.14, bed1)
bed2=f"{TMP}/bed2.wav"; synth_bed("pink", clip2_len, 200, 80, 0.22, bed2)
...
run([FF,"-y","-loop","1","-t",str(ED1),"-i",ENDCARD1,"-f","lavfi","-t",str(ED1),"-i","anullsrc=r=48000:cl=stereo",
     ...
     "-r",str(FPS),"-c:a","aac","-b:a","160k","-ar","48000","-ac","2","-shortest",eo1])

ED2=3.5
eo2=f"{TMP}/end2.mp4"
run([FF,"-y","-loop","1","-t",str(ED2),"-i",ENDCARD2,"-f","lavfi","-t",str(ED2),"-i","anullsrc=r=48000:cl=stereo",
     "-vf",f"scale={W}:{H},fps={FPS},format=yuv420p","-c:v","libx264","-preset","medium","-crf","18",
     "-r",str(FPS),"-c:a","aac","-b:a","160k","-ar","48000","-ac","2","-shortest",eo2])

lst=f"{TMP}/final_list.txt"
open(lst,"w").write(f"file '{main_out}'\nfile '{eo1}'\nfile '{eo2}'\n")
out=f"{OUT}/easy-taxi-belgium-ad-9x16-v15.mp4"
...
print("FINAL v15:",out,"dur",re.search(r"Duration: ([0-9:.]+)",d).group(1))
