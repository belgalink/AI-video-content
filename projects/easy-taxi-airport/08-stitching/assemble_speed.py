...
CL=f"{ROOT}/07-transition-videos/approved"; OUT=f"{ROOT}/09-final-output"
TMP="/tmp/asm2"; os.makedirs(TMP,exist_ok=True)
SPEED=1.22
# (file, start, dur)  dur = source seconds consumed; output = dur/SPEED
CLIPS=[(f"{CL}/shot1.mp4",2.2,2.6),(f"{CL}/shot2.mp4",1.4,2.4),(f"{CL}/shot3.mp4",2.0,2.3),
       (f"{CL}/shot4.mp4",0.7,2.0),(f"{CL}/shot5.mp4",2.4,2.3),(f"{CL}/shot6.mp4",1.6,2.8)]
...
for i,(f,ss,dur) in enumerate(CLIPS):
    o=f"{TMP}/s{i}.mp4"
    vf=f"scale={W}:{H}:force_original_aspect_ratio=increase,crop={W}:{H},setpts=PTS/{SPEED},fps={FPS},format=yuv420p"
    run([FF,"-y","-ss",str(ss),"-t",str(dur),"-i",f,"-vf",vf,
         "-af",f"atempo={SPEED},afade=t=in:st=0:d=0.05,afade=t=out:st=%.2f:d=0.06"%(dur/SPEED-0.06),
...
out=f"{OUT}/easy-taxi-belgium-ad-9x16-v2.mp4"
...
print("V2:",out,"dur",re.search(r"Duration: ([0-9:.]+)",d).group(1))
