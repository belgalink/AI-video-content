...
CL=f"{ROOT}/07-transition-videos/approved/kling"; OUT=f"{ROOT}/09-final-output"
TMP="/tmp/asmk"; os.makedirs(TMP,exist_ok=True)
# Kling clips already realistic; pace comes from tight windows. (file,start,dur)
CLIPS=[(f"{CL}/shot1.mp4",0.8,2.4),(f"{CL}/shot2.mp4",0.5,2.1),(f"{CL}/shot3.mp4",0.8,2.2),
       (f"{CL}/shot4.mp4",0.4,1.6),(f"{CL}/shot5.mp4",0.6,2.0),(f"{CL}/shot6.mp4",1.2,2.5)]
...
print("FINAL(Kling):",out,"dur",re.search(r"Duration: ([0-9:.]+)",d).group(1))
